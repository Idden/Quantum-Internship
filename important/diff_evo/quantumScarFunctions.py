...
def get_zero_scar(N, k0=None):
    ...
    # ARPACK requires k < D - 1. Without this clamp get_zero_scar(4) raises,
    # because D = 7 there but K starts at 16. That was documented in the test
    # suite as "N=4 needs an explicit k0"; it is a missing clamp, not a real
    # limitation. The doubling below is clamped for the same reason.
    K = max(1, min(K, D - 2))
