...
def get_scar_ham(N, fixed_seed=False, ohms=1.0, diagonalize=True, periodic=True):
    ...
    H0 = (ohms / 2 * sparseBareHamiltonian)
    H0 = qt.Qobj(H0)
...
def get_zero_scar(N, k0=None):
    ...
    # ARPACK requires k < D - 1. At small N the 0.02*D floor of 16 overshoots
    # the whole space, so clamp the start size and the doubling.
    K_MAX = D - 2
    if K_MAX < 1:
        # space too small for ARPACK at all: dense nullspace instead
        H2d = H2.toarray()
        w_all, v_all = np.linalg.eigh(H2d)
        nz = np.linalg.norm(Hx @ v_all, axis=0) < 1e-8
        v = v_all
    else:
        K = min(K, K_MAX)
        while True:
