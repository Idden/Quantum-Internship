...
class UserInterface():
    def __init__(self):
        ...
        # window details
        self.root = tk.Tk()
        self.root.title("Unitary Matrix")
        ...
        self.cb1 = tk.Checkbutton(self.frame, text="Print Unitary Matrix", variable=self.v1, font=("arial", 18))
...
# creates the random complex matrix
randomComplexMatrix = np.round(realPart + 1j * imagPart, sigFigs)

# creates the hermitian matrix
...
unitaryMatrix = np.round(expm(-1j * hermitianMatrix), sigFigs)

# creates the lists for the eigenvalues and eigenvectors
eigenvalues, eigenvectors = np.linalg.eig(unitaryMatrix)
...
# tests if unitarity is correct
#unitaryDagger = np.transpose(np.conj(unitaryMatrix))
#maybeIdentity = np.round(np.matmul(unitaryMatrix, unitaryDagger), 3)
...
xUnit = np.cos(theta)
yUnit = np.sin(theta)
plt.plot(xUnit, yUnit, c="red")

# the axes
axes = [i for i in range(-2, 3, 1)]
plt.xticks(axes)
plt.yticks(axes)
